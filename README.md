...
jam sprint fix-1 --start       # status shows (ready)/(blocked); a sprint can't --start until its needs are done
jam promote-sprint fix-9 --title "..." --reason "discovered during fix-1" --needs fix-1  # governed scope expansion (provenance: promoted)
...
jam audit                      # run-honesty audit: ordering + Codex authorship + evidence (also enforced at FINISH)
