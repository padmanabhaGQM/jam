...
export function recordBuildPlan(args) {
  const { runDir: dir, sprints, verifyCmd, finishCmd, now } = args;
  const hasFinishCmd = Object.prototype.hasOwnProperty.call(args, "finishCmd");
  ...
  const { valid, errors } = validatePlan({ verifyCmd: state.spec.verifyCmd, ...(hasFinishCmd ? { finishCmd } : {}), sprints });
  ...
  if (!hasFinishCmd) delete state.plan.finishCmd;
  else state.plan.finishCmd = finishCmd;
